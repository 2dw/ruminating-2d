...
export default function AlbumNavigation() {
  ...
  const items = useMemo(() => {
    return albums.map((album) => ({ album, meta: getAlbumMeta(album) }))
  }, [albums])
  ...
  // --- Map view: decluster overlapping markers while keeping true locations ---
  ...
  // --- Timeline view: real year range derived from album data ---
  const timeline = useMemo(() => {
    const ranged = items.filter((item) => item.meta.dateRange)
  ...
  const hovered = useMemo(() => items.find((i) => i.album.id === hoveredAlbum) || null, [items, hoveredAlbum])
      ...
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="flex gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800">
          <ToggleButton
            ...
            activeClass="bg-teal-600/80 text-white shadow-lg"
          >
            Constellation Map
          </ToggleButton>
          <ToggleButton
            ...
            activeClass="bg-green-600/80 text-white shadow-lg"
          >
            Life Timeline
          </ToggleButton>
        </div>
        <span className="text-xs text-slate-400">
          {loading ? "Loading albums..." : `${albums.length} albums`}
        </span>
      </div>

      {loading && albums.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 p-10 text-center text-sm text-slate-400 dark:border-slate-700">
          ...
          <div className="relative w-full" style={{ minWidth: 0 }}>
            ...
            >
              ...
              <g opacity={0.07} stroke="rgb(20, 184, 166)" strokeWidth={0.5}>
                {Array.from({ length: 12 }, (_, i) => (
                  <line key={`v${i}`} x1={((i + 1) * WORLD_MAP_W) / 12} y1={0} x2={((i + 1) * WORLD_MAP_W) / 12} y2={WORLD_MAP_H} />
                ))}
              ...
              {/* Leader lines from true location to decluttered marker */}
              {mapPoints.map((p) => {
                const moved = Math.hypot(p.x - p.ox, p.y - p.oy) > 6
              ...
              {mapPoints.map((p) =>
                Math.hypot(p.x - p.ox, p.y - p.oy) > 6 ? (
                  <circle key={`dot-${p.album.id}`} cx={p.ox} cy={p.oy} r={1.6} fill="rgba(45, 212, 191, 0.7)" />
                ) : null,
              )}
                ...
                return (
                  ...
                  >
                    <MushroomCap
                      ...
                      active={isHovered}
                      onClick={() => handleAlbumClick(p.album.id)}
            ...
            {hoveredMapPoint && hovered && (
              ...
              >
                ...
                {hovered.meta.location && (
                  <div className="text-slate-400">{hovered.meta.location.label}</div>
                )}
                ...
                return (
                  ...
                  >
                    ...
                    <MushroomCap
                      ...
                      color={color}
                      active={isHovered}
                      onClick={() => handleAlbumClick(entry.album.id)}
