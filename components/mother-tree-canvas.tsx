...
interface LeafNode { x: number; y: number; r: number; phase: number; branchIdx: number }
...
// ── Layout ──────────────────────────────────────────────────────────────────
const CANOPY_ZONE = [0.02, 0.48]
const TRUNK_ZONE  = [0.35, 0.62]
const ROOT_ZONE   = [0.58, 1.0]

// ── Tier clusters (positioned far out in canopy, spaced from tree) ──────────
const TIER_CLUSTERS: Omit<TierCluster, "cx" | "cy" | "leaves">[] = [
  { radius: 50, label: "Thought Pieces", href: "#thought-pieces", icon: "\u2606" },
  { radius: 55, label: "Deep Dives",     href: "#deep-dives",     icon: "\u26A1" },
]
...
function generateBgConnections(stars: Star[], maxDist: number, count: number): [number, number][] {
  ...
  for (let i = 0; i < stars.length && conns.length < count; i++) {
    for (let j = i + 1; j < stars.length && conns.length < count; j++) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < maxDist && srng() < 0.25) {
        conns.push([i, j])
      }
    }
  }
...
function generateTree(w: number, h: number): { branches: TreeBranch[]; leafNodes: LeafNode[] } {
  resetSeed()
  ...
  function grow(x: number, y: number, angle: number, len: number, thick: number, depth: number, parentIdx: number) {
    ...
    // Leaf nodes at branch tips and midpoints
    if (depth >= 2 && srng() < 0.6) {
      leafNodes.push({
        x: x2 + (srng() - 0.5) * 8,
        y: y2 + (srng() - 0.5) * 8,
    ...
    const shrink = 0.62 + srng() * 0.12
    if (srng() < 0.7) {
      grow(x2, y2, angle - spread, len * shrink, thick * 0.68, depth + 1, idx)
      grow(x2, y2, angle + spread, len * shrink, thick * 0.68, depth + 1, idx)
    } else {
      grow(x2, y2, angle + (srng() - 0.5) * 0.3, len * 0.78, thick * 0.78, depth + 1, idx)
    }
  }

  // Main trunk (thick, prominent)
  const trunkBase = cx + (srng() - 0.5) * w * 0.03
  const trunkTopX = cx + (srng() - 0.5) * w * 0.015
  branches.push({ x1: trunkBase, y1: trunkBot, x2: trunkTopX, y2: trunkTop + trunkH * 0.3, thickness: 7, depth: 0 })

  // Crown branches reaching high into canopy
  const crownY = trunkTop + trunkH * 0.3
  const crownSpread = w * 0.14
  grow(trunkTopX - crownSpread * 0.4, crownY, -Math.PI / 2 - 0.35, trunkH * 0.28, 4, 1, 0)
  grow(trunkTopX + crownSpread * 0.4, crownY, -Math.PI / 2 + 0.35, trunkH * 0.28, 4, 1, 0)
  grow(trunkTopX, crownY - trunkH * 0.06, -Math.PI / 2, trunkH * 0.32, 4.5, 1, 0)
  grow(trunkTopX - crownSpread * 0.15, crownY, -Math.PI / 2 - 0.55, trunkH * 0.22, 3, 1, 0)
  grow(trunkTopX + crownSpread * 0.15, crownY, -Math.PI / 2 + 0.55, trunkH * 0.22, 3, 1, 0)

  // Mid branches
  const midY = trunkBot - trunkH * 0.2
  grow(trunkBase - 3, midY, -Math.PI / 2 - 0.65, trunkH * 0.18, 2.8, 2, 0)
  grow(trunkBase + 3, midY, -Math.PI / 2 + 0.65, trunkH * 0.18, 2.8, 2, 0)

  // Extra crown density
  const crownHigh = trunkTop + trunkH * 0.15
  grow(trunkTopX - crownSpread * 0.25, crownHigh, -Math.PI / 2 - 0.25, trunkH * 0.18, 2.5, 2, 0)
  grow(trunkTopX + crownSpread * 0.25, crownHigh, -Math.PI / 2 + 0.25, trunkH * 0.18, 2.5, 2, 0)

  return { branches, leafNodes }
}

function generateRoots(w: number, h: number): Root[] {
  resetSeed()
  const roots: Root[] = []
  const origins = [0.36, 0.43, 0.5, 0.57, 0.64]
  ...
  function grow(x: number, y: number, angle: number, depth: number, path: [number, number][]) {
    ...
    const len = (12 + srng() * 20) * (1 - depth * 0.1)
    ...
    if (srng() < 0.6) {
      ...
      grow(nx, ny, angle + (srng() - 0.5) * 0.22, depth + 1, [...path])
    }
  }

  origins.forEach((ox) => {
    const startX = ox * w
    grow(startX, rootTop, Math.PI / 2 + (srng() - 0.5) * 0.4, 0, [[startX, rootTop]])
  })
...
function generateTierClusters(w: number, h: number): TierCluster[] {
  resetSeed()
  const canopyMidY = (CANOPY_ZONE[0] + CANOPY_ZONE[1]) / 2 * h
  const leftX = w * 0.15
  const rightX = w * 0.85

  return TIER_CLUSTERS.map((def, i) => {
    const cx = i === 0 ? leftX : rightX
    const cy = canopyMidY + (srng() - 0.5) * h * 0.1
...
export function MotherTreeCanvas({ summary, className }: MotherTreeCanvasProps) {
  ...
  const dataRef = useRef<{
    ...
    branches: TreeBranch[]; leafNodes: LeafNode[]
  ...
  const buildData = useCallback((w: number, h: number) => {
    ...
    const bgStarCount = isMobile ? 20 : 40
    const bgConnCount = isMobile ? 10 : 20
    const bgStars = generateBgStars(w, h, bgStarCount)
    const { branches, leafNodes } = generateTree(w, h)
    dataRef.current = {
      bgStars,
      bgConns: generateBgConnections(bgStars, w * 0.1, bgConnCount),
      branches,
      leafNodes,
      roots: generateRoots(w, h),
      tierClusters: generateTierClusters(w, h),
      particles: spawnParticles(w, h, isMobile ? 10 : 20),
      w, h,
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const { bgStars, bgConns, branches, leafNodes, roots, tierClusters, particles, w, h } = dataRef.current
      t += 0.012
      ...
      bgConns.forEach(([i, j]) => {
        ...
        ctx.moveTo(a.x, a.y)
        ctx.lineTo(b.x, b.y)
        ctx.strokeStyle = alpha(palette.lineStroke, 0.06)
        ctx.lineWidth = 0.3
        ctx.setLineDash([2, 5])
        ctx.stroke()
        ctx.setLineDash([])
      ...
      bgStars.forEach((s) => {
        const flicker = 0.4 + 0.6 * Math.sin(t * 0.8 + s.phase)
      ...
      particles.forEach((p) => {
        p.y -= p.speed
        p.x += Math.sin(t * 1.8 + p.phase) * 0.25
      ...
      roots.forEach((r) => {
        ...
        ctx.lineWidth = Math.max(0.3, 1.2 - r.depth * 0.2)
        ctx.lineCap = "round"
        ctx.stroke()
        if (r.depth >= 3) {
          ctx.beginPath()
          ctx.arc(last[0], last[1], 1.2, 0, Math.PI * 2)
          ctx.fillStyle = alpha(palette.rootNode, a * 0.5)
          ctx.fill()
        }
      })
      ...
      const rootTop = ROOT_ZONE[0] * h
      ;[0.36, 0.43, 0.5, 0.57, 0.64].forEach((ox) => {
        ctx.beginPath()
        ctx.arc(ox * w, rootTop, 2, 0, Math.PI * 2)
        ctx.fillStyle = alpha(palette.rootNode, 0.6)
        ctx.fill()
      })

      // ── Tree trunk and branches (prominent) ──
      branches.forEach((b) => {
        ctx.beginPath()
        ctx.moveTo(b.x1, b.y1)
        ctx.lineTo(b.x2, b.y2)
        const a = Math.max(0.4, 1 - b.depth * 0.1)
        ctx.strokeStyle = alpha(palette.trunkStroke, a)
        ctx.lineWidth = Math.max(0.6, b.thickness)
        ctx.lineCap = "round"
        ctx.stroke()

        // Subtle trunk glow for main branches
        if (b.depth <= 1) {
          ctx.beginPath()
          ctx.moveTo(b.x1, b.y1)
          ctx.lineTo(b.x2, b.y2)
          ctx.strokeStyle = alpha(palette.trunkGlow, a * 0.08)
          ctx.lineWidth = Math.max(1, b.thickness * 2.5)
          ctx.stroke()
        }
      })

      // ── Permanent leaf stars on tree branches ──
      const mx = mouseRef.current.x
      const my = mouseRef.current.y
      leafNodes.forEach((leaf) => {
        const distToMouse = Math.sqrt((mx - leaf.x) ** 2 + (my - leaf.y) ** 2)
        const isNearMouse = distToMouse < 40
        const flicker = 0.4 + 0.6 * Math.sin(t * 2.0 + leaf.phase)
        const a = isNearMouse ? 0.8 + 0.2 * flicker : 0.2 + 0.25 * flicker
        const r = isNearMouse ? leaf.r * 1.6 : leaf.r

        // glow
        ctx.beginPath()
        ctx.arc(leaf.x, leaf.y, r * (isNearMouse ? 5 : 3), 0, Math.PI * 2)
        ctx.fillStyle = alpha(palette.leafGlow, a * (isNearMouse ? 0.2 : 0.05))
        ctx.fill()
      ...
      // ── Tier constellation clusters (spaced out, on sides) ──
      tierClusters.forEach((cluster, ci) => {
        ...
        // Connecting lines between leaves
        for (let i = 0; i < cluster.leaves.length; i++) {
          for (let j = i + 1; j < cluster.leaves.length; j++) {
            ...
            if (d < cluster.radius * 0.85) {
              ctx.beginPath()
              ctx.moveTo(la.x, la.y)
              ctx.lineTo(lb.x, lb.y)
              ctx.strokeStyle = alpha(palette.leafRing, isHovered ? 0.3 : 0.05)
              ctx.lineWidth = isHovered ? 0.8 : 0.25
              ctx.stroke()
            }
          }
        }
        ...
        cluster.leaves.forEach((leaf) => {
          const flicker = 0.4 + 0.6 * Math.sin(t * 1.6 + leaf.phase)
          const a = isHovered ? 0.8 + 0.2 * flicker : 0.12 + 0.15 * flicker
          const r = isHovered ? leaf.r * 1.5 : leaf.r

          ctx.beginPath()
          ctx.arc(leaf.x, leaf.y, r * (isHovered ? 5 : 2.5), 0, Math.PI * 2)
          ctx.fillStyle = alpha(palette.leafGlow, a * (isHovered ? 0.22 : 0.04))
          ctx.fill()
        ...
        if (isHovered) {
          ctx.beginPath()
          ctx.arc(cluster.cx, cluster.cy, cluster.radius * 0.4, 0, Math.PI * 2)
          ctx.fillStyle = alpha(palette.leafGlow, 0.05)
          ctx.fill()
        }
      })
      ...
      <AnimatePresence>
        {activeCluster && (
          <motion.div
            ...
            style={{ left: activeCluster.cx, top: activeCluster.cy - activeCluster.radius - 24 }}
          >
